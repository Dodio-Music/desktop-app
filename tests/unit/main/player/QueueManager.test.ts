...
describe('QueueManager Strategies', () => {
    const mockTracks: BaseSongEntry[] = [
        {
            ...
            duration: 100,
            type: 'local',
            context: {type: 'local', name: 'L', url: 'u'}
        },
        {
            id: '2',
            title: 'Song 2',
    ...
    describe('RepeatOffStrategy', () => {
        ...
        it('returns requested count of songs', () => {
            const state: QueueState = {
                current: mockTracks[0],
                userQueue: [mockTracks[1], mockTracks[2]],
                context: {tracks: mockTracks, startIndex: 0}
            };
            const upcoming = strategy.getUpcoming(state, 1);
...
describe('QueueManager', () => {
    let mockWindow: {isDestroyed: Mock<() => boolean>, webContents: {send: (...args: unknown[]) => void}};
    ...
    beforeEach(() => {
        ...
        queueManager = new QueueManager(mockWindow as unknown as BrowserWindow, RepeatMode.All);
        vi.clearAllMocks(); // Clear the call from the constructor
    });
    ...
    it('advances to next song', () => {
        ...
        expect(queueManager.getNext()?.id).toBe('3');
        expect(queueManager.next()?.id).toBe('3');
        expect(queueManager.next()?.id).toBe('1'); // Wraps because RepeatMode.All
    });

    it('handles user queue with priority', () => {
        const userSong: BaseSongEntry = {
            id: 'user-1',
            title: 'User Song',
            artists: [],
            album: 'A',
    ...
    it('returns null on previous if at start and RepeatMode.Off', () => {
    ...
    it('does not notify if window is destroyed', () => {
        mockWindow.isDestroyed.mockReturnValue(true);
        // Create new manager with destroyed window
        const destroyedManager = new QueueManager(mockWindow as unknown as BrowserWindow, RepeatMode.Off);
        expect(mockWindow.webContents.send).not.toHaveBeenCalled();

        // Also try calling it explicitly
        destroyedManager.setRepeatMode(RepeatMode.One);
        expect(mockWindow.webContents.send).not.toHaveBeenCalled();
    });

    it('gets upcoming count correctly', () => {
        queueManager.setContext(mockTracks, 0);
        const upcoming = queueManager.getUpcoming(2);
